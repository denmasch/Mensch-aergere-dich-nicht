...
namespace MadnServer.Services
{
    public class StatsService
    {
        ...
        {
            await PersistMatch(gameId, endTime, gameStatus: GameStatus.Completed, winnerPlayerId, winnerColor);
        }

        public async Task CancelMatch(Guid gameId, DateTime endTime)
        {
            await PersistMatch(gameId, endTime, gameStatus: GameStatus.Canceled);
        }

        private async Task PersistMatch(Guid gameId, DateTime endTime, GameStatus gameStatus, Guid? winnerPlayerId = null, Color? winnerColor = null)
        {
            ...
            {
                Logger.LogError($"Failed to persist match stats for {gameId}: {ex.Message}");
            }
            finally
            {
                sem.Release();
        ...
        // Read stored match files and return deserialized list
        public List<MatchStats> GetStoredMatches()
        {
            ...
            try
            {
                ...
                foreach (var f in files)
                {
                    ...
                    {
                        Logger.LogError($"Failed to read match file {f}: {ex.Message}");
                    }
                }
            }
            catch (Exception ex)
            {
                Logger.LogError($"Failed to enumerate match files in {_outDir}: {ex.Message}");
            }
